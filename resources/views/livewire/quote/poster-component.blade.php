<div>
    <div class="p-6 bg-white rounded-lg shadow-lg">
        <x-form-section submit="calcular">
            <x-slot name="title">
                {{ __('Cálculo de Cotización de Poster') }}
            </x-slot>
        ...
        <x-dialog-modal wire:model="showModal">
            ...
            <x-slot name="content">
                @if($resultados)
                    <div>
                        <p><strong>{{ __('Moldura:') }}</strong> {{ $resultados['medida_moldura'] }} mt lineales - ${{ $resultados['precio_moldura'] }}</p>
                        <p><strong>{{ __('Trupán:') }}</strong> {{ $resultados['medida_trupan'] }} mt cuadrados - ${{ $resultados['precio_trupan'] }}</p>
                        <p><strong>{{ __('Montaje:') }}</strong> ${{ $resultados['montaje'] }}</p>
                        <p><strong>{{ __('Subtotal:') }}</strong> ${{ $resultados['subtotal'] }}</p>
                        <p><strong>{{ __('Gastos Generales:') }}</strong> ${{ $resultados['gastos_grles'] }}</p>
                        <p><strong>{{ __('Subtotal con Gastos:') }}</strong> ${{ $resultados['subtotal_con_gastos'] }}</p>
                        <p><strong>{{ __('Descuento:') }}</strong> ${{ $resultados['descuento'] }}</p>
                        <p><strong>{{ __('Total:') }}</strong> ${{ $resultados['total'] }}</p>
                        <p><strong>{{ __('IVA:') }}</strong> ${{ $resultados['iva'] }}</p>
                        <p><strong>{{ __('Total General:') }}</strong> ${{ $resultados['total_general'] }}</p>
                    </div>
                @endif
            </x-slot>
